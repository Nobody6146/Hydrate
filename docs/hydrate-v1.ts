/*
    With Hydrate1, you can bring life to your static web page by utilizing dynamic data and reactive components.
    Powerful features like 2-way modeling binding and client-side routing come out of the box.
*/
//******** Options *******//
/** Options for Hydrate1 DOM manipulation */
class Hydrate1DOMOptions
{
    root: string;
    attributePrefix: string;
    attributes: Hydrate1HTMLAttributeOptions;
    modifiers: Hydrate1HTMLAttributeModifiers;

    constructor()
    {
        this.root = "body";
        this.attributePrefix = "h";
        this.attributes = new Hydrate1HTMLAttributeOptions();
        this.modifiers = new Hydrate1HTMLAttributeModifiers();
    }
}
class Hydrate1HTMLAttributeOptions
{
    //Basic
    model = "model";
...
class Hydrate1HTMLAttributeModifiers
{
    array: string;
    dictionary: string;
    model:string;
...
/** Options for Hydrate1 routing */
class Hydrate1RouterOptions
{
    hashRouting: boolean;
    exactMatch: boolean;
...
/** Hydrate1 app options */
class Hydrate1AppOptions
{
    dom: Hydrate1DOMOptions;
    router: Hydrate1RouterOptions;
    models: Hydrate1ModelOptions;

    constructor()
    {
        this.dom = new Hydrate1DOMOptions();
        this.router = new Hydrate1RouterOptions();
        this.models = new Hydrate1ModelOptions();
    }
}
//****************//
//********Events********//
type Hydrate1ModelEventType = 'bind' | "unbind" | 'set' | 'callback' | 'function' | 'handler' | 'initialize' | 'route';
type Hydrate1DispatchMode = 'domOnly' | 'subscriptionsOnly' | 'all';

class Hydrate1ModelEvent {
    Hydrate1: Hydrate1App;
    target: HTMLElement;
    type: Hydrate1ModelEventType;
    ...
    constructor(type:Hydrate1ModelEventType, model: any, previousState: any, propName: string, target: HTMLElement, Hydrate1: Hydrate1App, modelName?: string)
    {
        this.Hydrate1 = Hydrate1;
        this.target = target;
        this.type = type;
        this.model = model;
        this.modelName = Hydrate1.name(model) ?? modelName;
        this.rootModelName = this.modelName != null 
            ? this.modelName.split(Hydrate1.options.models.nestedOperator)[0]
            : undefined;
        this.state = Hydrate1.state(model) ?? model;
        this.previousState = previousState;
        this.propName = propName;
        this.propFullName = (this.modelName != null && this.propName != null)
            ? this.modelName + Hydrate1.options.models.nestedOperator + this.propName
            : undefined;
...
//****************//
class Hydrate1DomAttributeArguments
{
    arg1: string;
    arg2: string;
    arg3: string;
...
class Hydrate1DomUpdateAttributes
{
    model: Hydrate1DomAttributeArguments[];
    attribute: Hydrate1DomAttributeArguments[];
    property: Hydrate1DomAttributeArguments[];
    toggle: Hydrate1DomAttributeArguments[];
    class: Hydrate1DomAttributeArguments[];
    delete: Hydrate1DomAttributeArguments[];

    input: Hydrate1DomAttributeArguments[];

    callback: Hydrate1DomAttributeArguments[];
    handler: Hydrate1DomAttributeArguments[];
    function: Hydrate1DomAttributeArguments[];
    dispatch: Hydrate1DomAttributeArguments[];

    script: Hydrate1DomAttributeArguments[];
    template: Hydrate1DomAttributeArguments[];
    initialize: Hydrate1DomAttributeArguments[];
    component: Hydrate1DomAttributeArguments[];

    route: Hydrate1DomAttributeArguments[];
    page: Hydrate1DomAttributeArguments[];

    event: Hydrate1DomAttributeArguments[];
    static: Hydrate1DomAttributeArguments[];
    condition: Hydrate1DomAttributeArguments[];
}

class Hydrate1DeterminePropResult
{
    success: boolean;
    prop: any;
    propName: string;
...
class Hydrate1Route {
    path:string;
    callback: (req:Hydrate1RouteRequest, res:Hydrate1RouteResponse) => Promise<any>;
}

class Hydrate1RouterContext {
    path: string;
    url: string;
    pathname: string;
    search: string;
...
class Hydrate1App
{
    ...
    #eventListeners : Map<string, ((event: Hydrate1ModelEvent) => any)[]>;
    #observer : MutationObserver;
    #routes: Hydrate1Route[];
    #routerContext: Hydrate1RouterContext;
    options: Hydrate1AppOptions;

    constructor(options?: Hydrate1AppOptions)
    {
        this.options = options ?? new Hydrate1AppOptions();
        this.#models = new Map();
        ...
        //Start the router
        this.#routes = [];
        this.#routerContext = new Hydrate1RouterContext();
    ...
    refresh(mode?: Hydrate1DispatchMode) {
    ...
    subscriptions(search: string | object, handler: (event: Hydrate1ModelEvent) => any) :{modelName:string, handler: (event: Hydrate1ModelEvent) => any}[] {
        ...
        let results:{modelName:string, handler: (event: Hydrate1ModelEvent) => any}[] = [];
    ...
    subscribe(search : string | object, handler: (event: Hydrate1ModelEvent) => any) : (event: Hydrate1ModelEvent) => any
    {
        const name = (typeof search === "string")
            ? search
            : this.name(search);
    ...
    unsubscribe(search: string | object, handler: (event: Hydrate1ModelEvent) => any) : (event: Hydrate1ModelEvent) => any
    {
        const name = (typeof search === "string")
            ? search
            : this.name(search);
    ...
    //======== Routing =============//
    //Queries the routes
    routes(path: string, handler: (req:Hydrate1RouteRequest, res:Hydrate1RouteResponse) => Promise<any>) :Hydrate1Route[] {
        return this.#routes.filter(route => 
            (path == null || route.path === path) && (handler == null || handler === route.callback)
        );
    }
    route(path:string, callback: (req:Hydrate1RouteRequest, res:Hydrate1RouteResponse) => any | Promise<any>): Hydrate1Route {
        let route:Hydrate1Route = {
            path: path,
            callback: callback
        };
    ...
    async dispatch(type: Hydrate1ModelEventType,  model: any, propName: string, previousState: any, target: HTMLElement, mode:Hydrate1DispatchMode, modelName?:string): Promise<void> {
        ...
        let eventMappings = new Map<string, Hydrate1ModelEvent[]>();
    ...
    #generateEvents(type: Hydrate1ModelEventType, target: HTMLElement, modelName: string, model: any, propName: string, searchKey: string, previousState: any) : Hydrate1ModelEvent[] {
        ...
        {
            return [new Hydrate1ModelEvent(type, localModel, localPreviousState, propName, target, this)];
        }
        else
        {
            return [new Hydrate1ModelEvent(type, localModel, localPreviousState, undefined, target, this, name)]
        };
    }
    ...
    async #dom(event: Hydrate1ModelEvent, target: HTMLElement, routerContext: Hydrate1RouterContext): Promise<void>
    {
        try {
            if(target == null)
                target = this.root;
            ...
            let routeRequest: Hydrate1RouteRequest;
    ...
    #loadDomAttributes(target : HTMLElement): Hydrate1DomUpdateAttributes {
        const app = this;
        let getArguments = function(target: HTMLElement, name: string): Hydrate1DomAttributeArguments[] {
            let attribute = target.attributes[app.attribute(name)];
            if(attribute === undefined)
                return undefined;
            return app.#splitAttributeValue(attribute.value.trim());
        }
        let attributes = new Hydrate1DomUpdateAttributes();
    ...
    #splitAttributeValue(value: string): Hydrate1DomAttributeArguments[] {
        if(value == null)
            return undefined;
        ...
        return value.split(';').map(x => { 
            ...
            if(!regParams)
            {
                ...
                return new Hydrate1DomAttributeArguments(args[0], args[1], undefined);
            }
            //We have a 3rd argument for a replacement/insertion value to stick into arg 2 field
            let result = baseParams.split(/\s+/);
            return new Hydrate1DomAttributeArguments(result[0], result[1], baseParams.substring(regParams[0].length));
        });
    }
    //******** HTML DOM MANIUPLATIONS *****//
    #determinePropValue(event:Hydrate1ModelEvent, arg: Hydrate1DomAttributeArguments):Hydrate1DeterminePropResult {
        ...
        if(event.propName === undefined)
        {
            if(arg.arg1 !== wildcard)
            {
                ...
                {
                    return new Hydrate1DeterminePropResult(event.state[arg.arg1], arg.arg1);
                }
                else
                    return new Hydrate1DeterminePropResult();
            }
            else {
                if(event.state instanceof Object)
                {
                    ...
                    return new Hydrate1DeterminePropResult(keys[key], key.toString());
                }
                else
                    return new Hydrate1DeterminePropResult(event.state, undefined);
            }
        }
        else if(arg.arg1 === event.propName || arg.arg1 === wildcard)
            return new Hydrate1DeterminePropResult(event.prop, event.propName);
        //This event doesn't match, so ignore
        else 
            return new Hydrate1DeterminePropResult();
    }
    #createLocalizedEvent(target: HTMLElement, event:Hydrate1ModelEvent, propName: string) {
        let type = event.type;
        ...
        //     }
        // }
        return new Hydrate1ModelEvent(type, event.model, event.previousState, propName, target, event.Hydrate1, event.modelName);
    }
    #getScriptValue(expression:string, event:Hydrate1ModelEvent, prop, route: Hydrate1RouteRequest) {
    ...
    #getDomRouteRequest(event: Hydrate1ModelEvent, attributes: Hydrate1DomUpdateAttributes, routerContext: Hydrate1RouterContext): {title:string, request: Hydrate1RouteRequest} {
    ...
    async #insertComponent(template: Node[], event: Hydrate1ModelEvent, propNames:string[], append:boolean, routeRequest:Hydrate1RouteRequest) {
    ...
    #updateHTMLAttribute(target: HTMLElement, event: Hydrate1ModelEvent, attributes: Hydrate1DomUpdateAttributes, routeRequest: Hydrate1RouteRequest): boolean {
        let updated = false;
        attributes.attribute.forEach(arg => {
            //We don't have 
            if(arg.arg2 === undefined)
            {
                console.error(`missing arg2 for Hydrate1 attribute for element ${target}`);
                return false;
            }
    ...
    #updateHTMLProperty(target: HTMLElement, event: Hydrate1ModelEvent, attributes: Hydrate1DomUpdateAttributes, routeRequest: Hydrate1RouteRequest): boolean {
        let updated = false;
        attributes.property.forEach(arg => {
            //We don't have 
            if(arg.arg2 === undefined)
            {
                console.error(`missing arg2 for Hydrate1 property for element ${target}`);
                return false;
            }
    ...
    #updateHTMLToggleAttribute(target: HTMLElement, event: Hydrate1ModelEvent, attributes: Hydrate1DomUpdateAttributes, routeRequest: Hydrate1RouteRequest): boolean {
        let updated = false;
        attributes.toggle.forEach(arg => {
            //We don't have 
            if(arg.arg2 === undefined)
            {
                console.error(`missing arg2 for Hydrate1 toggle for element ${target}`);
                return false;
            }
    ...
    #updateHTMLToggleClass(target: HTMLElement, event: Hydrate1ModelEvent, attributes: Hydrate1DomUpdateAttributes, routeRequest: Hydrate1RouteRequest): boolean {
        let updated = false;
        attributes.class.forEach(arg => {
            //We don't have 
            if(arg.arg2 === undefined)
            {
                console.error(`missing arg2 for Hydrate1 toggle class for element ${target}`);
                return false;
            }
    ...
    #updateHTMLDeleteElement(target: HTMLElement, event: Hydrate1ModelEvent, attributes: Hydrate1DomUpdateAttributes, routeRequest: Hydrate1RouteRequest): boolean {
        let updated = false;
    ...
    #updateHTMLCallback(target: HTMLElement, event: Hydrate1ModelEvent, attributes: Hydrate1DomUpdateAttributes, routeRequest: Hydrate1RouteRequest): boolean {
        let updated = false;
        attributes.callback.forEach(arg => {
            //We don't have 
            if(arg.arg2 === undefined)
            {
                console.error(`missing arg2 for Hydrate1 callback for element ${target}`);
                return false;
            }
            //Determine the value we need to evaluate
            let propResult = this.#determinePropValue(event, arg);
    ...
    #updateHTMLFunction(target: HTMLElement, event: Hydrate1ModelEvent, attributes: Hydrate1DomUpdateAttributes, routeRequest: Hydrate1RouteRequest): boolean {
        let updated = false;
        attributes.function.forEach(arg => {
            //We don't have 
            if(arg.arg2 === undefined)
            {
                console.error(`missing arg2 for Hydrate1 function for element ${target}`);
                return false;
            }
            //Determine the value we need to evaluate
            let propResult = this.#determinePropValue(event, arg);
    ...
    #updateHTMLHandler(target: HTMLElement, event: Hydrate1ModelEvent, attributes: Hydrate1DomUpdateAttributes, routeRequest: Hydrate1RouteRequest): boolean {
        let updated = false;
        attributes.handler.forEach(arg => {
            //We don't have 
            if(arg.arg2 === undefined)
            {
                console.error(`missing arg2 for Hydrate1 handler for element ${target}`);
                return false;
            }
            ...
            else
            {
                ...
                target[arg.arg2] = (elementEvent) => {
                    ...
                    let localEvent = new Hydrate1ModelEvent("handler", model, app.state(model), propResult.propName, target, app, app.name(model));
                    let prop = this.#getScriptValue(arg.arg3, localEvent, undefined, routeRequest);
    ...
    #updateHTMLDispatch(target: HTMLElement, event: Hydrate1ModelEvent, attributes: Hydrate1DomUpdateAttributes, routeRequest: Hydrate1RouteRequest): boolean {
        let updated = false;
        let app = this;
        attributes.dispatch.forEach(arg => {
            ...
            if(arg.arg2 === undefined)
            {
                console.error(`missing arg2 for Hydrate1 dispatch for element ${target}`);
                return false;
            }
            //Determine the value we need to evaluate
            let propResult = this.#determinePropValue(event, arg);
            ...
            propNames.forEach(propName => {
                ...
                app.dispatch(<Hydrate1ModelEventType>arg.arg2, app.model(mName), pName, app.state(mName), target, "all");
                updated = true;
            });
        });
        return updated;
    }
    async #updateHTMLComponent(target: HTMLElement, event: Hydrate1ModelEvent, attributes: Hydrate1DomUpdateAttributes, routeRequest: Hydrate1RouteRequest): Promise<boolean> {
        let updated = false;
        try {
            attributes.component.forEach(async (arg) => {
                ...
                if(arg.arg2 === undefined)
                {
                    console.error(`missing arg2 for Hydrate1 component for element ${target}`);
                    return false;
                }
                ...
                propNames.forEach(async (propName) => {
                    ...
                    else
                    {
                        ...
                        let previousSate = (localEvent.previousState instanceof Object)
                            ...
                            : undefined;
                        let componentEvent = new Hydrate1ModelEvent(localEvent.type, model, previousSate, undefined, target, this, this.name(model));
    ...
    #matchRoute(path:string, routerContext:Hydrate1RouterContext): RegExpMatchArray {
        return routerContext.path.match(this.#pathToRegex(path));
    }
    #generateRouteRequest(path:string, match:RegExpMatchArray, routerContext:Hydrate1RouterContext): Hydrate1RouteRequest {
        return {
            route: path,
    ...
    async #navigateToRoute(state) {
        ...
        let routerContext:Hydrate1RouterContext = {
        ...
        const continuePipeline = async function() {
            ...
            let req:Hydrate1RouteRequest = app.#generateRouteRequest(match.route.path, match.result, routerContext);
            let res:Hydrate1RouteResponse = {
                Hydrate1: app,
    ...
    #routeDom(routeContext: Hydrate1RouterContext) {
